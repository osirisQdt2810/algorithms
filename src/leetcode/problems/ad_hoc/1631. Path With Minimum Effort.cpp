...
class Solution {
    private:
        ...
        bool canReachDestination(int r, int c, const std::vector<std::vector<int>>& heights,
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (abs(heights[r][c] - heights[r_n][c_n]) > threshold)
                    continue; // prune the edge whose weight is larger than threshold
                // visited the new node, if we can reach the destination through this child node, the task is done
                if (canReachDestination(r_n, c_n, heights, visited)) return true;
            }
            return false;
        }
    ...
    public:
        inline int minimumEffortPath_dijikstra(std::vector<std::vector<int>>& heights) {
            ...
            auto cmp = [](const WeightNeighbor& lhs, const WeightNeighbor& rhs) {
                return lhs.second > rhs.second;
            };
            std::priority_queue<WeightNeighbor, std::vector<WeightNeighbor>, decltype(cmp)> pq(
                cmp); // min-heap
